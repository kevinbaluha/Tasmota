...
void MLX90614_Every_Second(void)
{
 ...
 const char HTTP_IRTMP[] PROGMEM =
  "{s}MXL90614 " "OBJ-" D_TEMPERATURE "{m}%s " D_UNIT_DEGREE "%c{e}"
  "{s}MXL90614 " "AMB-" D_TEMPERATURE "{m}%s " D_UNIT_DEGREE "%c{e}";
#endif  // USE_WEBSERVER
...
void MLX90614_Show(uint8_t json)
{
  ...
  float obj_temperature = ConvertTemp(mlx90614.obj_temp);
  float amb_temperature = ConvertTemp(mlx90614.amb_temp);

  dtostrfd(obj_temperature, Settings.flag2.temperature_resolution, obj_tstr);

  char amb_tstr[16];
  dtostrfd(amb_temperature, Settings.flag2.temperature_resolution, amb_tstr);

  if (json) {
    ResponseAppend_P(PSTR(",\"MLX90614-OBJ\":{\"" D_JSON_TEMPERATURE "\":%s},\"MLX90614-AMB\":{\"" D_JSON_TEMPERATURE "\":%s}"), obj_tstr, amb_tstr);
#ifdef USE_WEBSERVER
  } else {
    WSContentSend_PD(HTTP_IRTMP, obj_tstr, TempUnit(), amb_tstr, TempUnit());
#endif
  }
}
